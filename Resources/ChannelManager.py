...
class ChannelManager(RedisResource):
    ...
    def create_channel(self) -> Channel:
        ...
        if self.r.get(channel_id) == None:
            _channel = Channel(channel_id=channel_id, ttl=60, time_created=str(datetime.now()), messages=[])
            self.r.set(channel_id, json.dumps(_channel.dict())) # copying the entire list feels bad...
